...
# I don't see other groups building a gps clock to make software work.
# However, I might have the program check online for the ntis clock
# just in case it is online.
# 
# Therefore not all the code will be forked over from 152i.
# 
...
# Forked Project 
    ...
    #               Future: Adding interoperability with other programs such as HRDeluxe Free, FLDigi, Rigserve etc if we can?
...
class SQDB:
    ...
    def log(self,n):                        # add item to journal logfile table (and other tables...)
        ...
        if n.band == '*QST': print("QST\a "+n.rept) # emit BEEP sound on QST
...
class newparticipantdialog:
    ...
    def dialog(self):
        ...
        ## 1. Check data - if one is filled all have to be exept call
        ...
        ## 5. Quit is scary, prefer a close button. 
        ...
        s.t = Toplevel(root)
        s.t.transient(root)
        s.t.title('FDLOG_SCICSG Add New Participant')
        f1 = Frame(s.t)
        ...
        Button(f2,text='Close',font=fdbfont,command=s.quitbtn)\
...
def renew_title():
    ...
    h = mob/60
    m = mob%60
    root.title('  FDLOG_SCICSG %s %s %s (Node: %s Time on Band: %d:%02d) %s:%s UTC %s/%s'%\
...
def updateqct():
    ...
    else: t = "Alone? Not receiving data from others."
    if net.send_errs: t = "SEND FAIL - Not sending data to others."; net.send_errs = 0
    if authk == '':   t = "NO AUTHKEY SELECTED"
    if node == '': t = "NO NODE SELECTED"
    if t: lblnet.configure(text=t,background='yellow')
    else: lblnet.configure(text="Network OK", background='grey')
...
# buttons too small
# local band button is also yellow
#(backgrounds on buttons problematic on mac)
...
if node == "":
    ...
    print "  Please be 7 characters! (I will add a letter to be unique)"
    ...
    if k != 'gota':
        while len(k) != 7:
            if k == 'gota':
                ...
                ## the smartest guy I know said this was horrible
                ## 1. if there is more than three characters
                ## 2. add the rest with "a"
...
# These root commands were removed without an explaination in the beta.
    ...
    .edit <call>       edit (in work)
        ...
        if (ch == '\r'):
            ...
            if stat == 5:                               # whole qso parsed
                ...
                ## Added database protection against no band, no power, no op, and no logger -- KD4SIR Scott Hibbs 7/15/2013 
